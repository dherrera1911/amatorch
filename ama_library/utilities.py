...
def fit_multiple_seeds(nEpochs, model, trainDataLoader, lossFun, opt_fun,
    ...
    Outputs:
    ----------------
      - minLoss: Loss vector for the lowest loss model among all seeds. (nEpochs)
      - minElapsed: Elapsed time vector for the seed with minimum loss. (nEpochs)
...
def fit_by_pairs(nEpochs, model, trainDataLoader, lossFun, opt_fun,
    ...
    Outputs:
    ----------------
      - trainingLoss: List with loss vectors for each pair of filters trained.
          list of length nPairs, each element is a vector of length nEpochs.
      - elapsedTimes: List with elapsed time vectors for each pair of filters
          trained. list of length nPairs, each element is a vector of length
    """
    if model.filtNorm == 'narrowband':
        sAmp = model.compute_normalized_stimulus_amplitude_spec(s=sAll,
                sameAsInit=True)
