...
class TestAMAGaussResponses:
    def test_response_values(self, ama_gauss, data, filters, responses_ref):
        ...
        with torch.no_grad():
            responses = ama_gauss.get_responses(data["stimuli"])
    ...
    def test_response_shapes(self, ama_gauss, data, filters, responses_ref):
        ...
        with torch.no_grad():
            responses_single = ama_gauss.get_responses(stimulus_single)
        ...
        with torch.no_grad():
            responses_2d = ama_gauss.get_responses(stimuli_2d)
...
class TestAMAGaussInference:
    def test_inference_values(
        self, ama_gauss, data, filters, log_likelihoods_ref, posteriors_ref, estimates_ref
    ):
        ...
        with torch.no_grad():
            log_likelihoods = ama_gauss.get_log_likelihoods(data["stimuli"])
            posteriors = ama_gauss.get_posteriors(data["stimuli"])
            estimates = ama_gauss.get_estimates(data["stimuli"])
    ...
    def test_inference_shapes(
        ...
        stimulus_single = data["stimuli"][0]
        with torch.no_grad():
            log_likelihoods_single = ama_gauss.get_log_likelihoods(stimulus_single)
            posteriors_single = ama_gauss.get_posteriors(stimulus_single)
            estimates_single = ama_gauss.get_estimates(stimulus_single)
        ...
        stimuli_2d = data["stimuli"][:100].reshape(2, 50, n_channels, n_dim)
        with torch.no_grad():
            log_likelihoods_2d = ama_gauss.get_log_likelihoods(stimuli_2d)
            posteriors_2d = ama_gauss.get_posteriors(stimuli_2d)
            estimates_2d = ama_gauss.get_estimates(stimuli_2d)
